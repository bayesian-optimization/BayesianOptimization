...
class BayesianOptimization(object):
    ...
    def __init__(self, f, pbounds, verbose=1):
        ...
        self.bounds = []
        for key in list(self.pbounds.keys()):
    ...
    def init(self, init_points):
        ...
        self.init_points += list(map(list, list(zip(*l))))
        ...
        for x in self.init_points:
            ...
            y_init.append(self.f(**dict(list(zip(self.keys, x)))))
    ...
    def explore(self, points_dict):
        ...
        self.init_points = list(map(list, list(zip(*all_points))))
    ...
    def set_bounds(self, new_bounds):
        ...
        for row, key in enumerate(list(self.pbounds.keys())):
    ...
    def maximize(self,
        ...
        for i in range(n_iter):
            ...
            self.Y = np.append(self.Y, self.f(**dict(list(zip(self.keys, x_max)))))
            ...
            self.res['max'] = {'max_val': self.Y.max(),
                               'max_params': dict(list(zip(self.keys,
                                                      self.X[self.Y.argmax()])))
                               }
            self.res['all']['values'].append(self.Y[-1])
            self.res['all']['params'].append(dict(list(zip(self.keys, self.X[-1]))))
