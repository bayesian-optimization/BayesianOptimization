...
class SequentialDomainReductionTransformer(DomainTransformer[Any]):
    ...
    def _update(self, target_space: TargetSpace[Any]) -> None:
    ...
    def _trim(
        self, new_bounds: NDArray[np.float64], global_bounds: NDArray[np.float64]
    ) -> NDArray[np.float64]:
        """
        ...
        for i, pbounds in enumerate(new_bounds):
            # If the one of the bounds is outside the global bounds,
            # reset the bound to the global bound
            # This is expected to happen when the window is near the global bounds,
        ...
        # Adjust new_bounds to ensure they respect the minimum window width
        # for each parameter
        for i, pbounds in enumerate(new_bounds):
    ...
    def _window_bounds_compatibility(self, global_bounds: NDArray[np.float64]) -> None:
    ...
    def _create_bounds(
        self, parameters: Iterable[str], bounds: NDArray[np.float64]
    ) -> dict[str, NDArray[np.float64]]:
