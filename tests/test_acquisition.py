...
def random_state():
    """
    Creates a NumPy RandomState instance with a fixed seed for reproducible results.
    
    Returns:
        numpy.random.RandomState: Random number generator seeded with 0.
    """
...
def constrained_target_space(target_func):
    """
    Creates a TargetSpace with parameter bounds and a constraint on the sum of 'x' and 'y'.
    
    The returned TargetSpace enforces that x + y is between 0.0 and 1.0, inclusive.
    """
...
class MockAcquisition(acquisition.AcquisitionFunction):
    def __init__(self):
        """
        Initializes the MockAcquisition instance for testing purposes.
        """
        super().__init__()

    def _get_acq(self, gp, constraint=None):
        """
        Returns a mock acquisition function for testing that computes a fixed quadratic form.
        
        The returned function ignores the Gaussian process and constraint inputs, and evaluates
        the acquisition value as (3 - x[0])^2 + (1 - x[1])^2 for each input point x.
        """
    ...
    def set_acquisition_params(self, params: dict) -> None:
        """
        Sets the parameters for the acquisition function.
        
        This method should be implemented by subclasses to update internal parameters
...
def test_acquisition_optimization(gp, target_space):
    """
    Tests that MockAcquisition suggests the correct next point and raises an error when no optimization strategies are enabled.
    """
    acq = MockAcquisition()
...
def test_acquisition_optimization_only_random(gp, target_space, random_state):
    """
    Tests that acquisition optimization using only random sampling suggests the correct point.
    
    Verifies that the suggestion from the acquisition function matches the expected value when only random sampling is used, and ensures that the best random sample is included among the seed points.
    """
    acq = MockAcquisition()
    target_space.register(params={"x": 2.5, "y": 0.5}, target=3.0)
...
def test_acquisition_optimization_only_l_bfgs_b(gp, target_space):
    """
    Tests that the acquisition function suggests the correct point using only L-BFGS-B optimization.
    
    Verifies that when random sampling is disabled and only L-BFGS-B optimization is used, the suggested point matches the expected optimum.
    """
    acq = MockAcquisition()
    target_space.register(params={"x": 2.5, "y": 0.5}, target=3.0)
...
def test_upper_confidence_bound(gp, target_space, random_state):
    """
    Tests the behavior of the UpperConfidenceBound acquisition function.
    
    Verifies parameter initialization, error handling when the Gaussian process is unfitted, correct suggestion of new points, and proper decay of the exploration parameter `kappa` after multiple suggestions.
    """
...
def test_smart_minimize_fails(target_space, random_state):
    """
    Tests that `_smart_minimize` returns infinity when the objective function produces NaN values.
    """
    acq = acquisition.UpperConfidenceBound()

    def fun(x):
        """
        Returns an array of NaNs matching the shape of the first column of `x`, or a single NaN if `x` is not two-dimensional.
        
        Args:
            x: Input array.
        
        Returns:
            An array of NaNs with the same shape as `x[:, 0]`, or a scalar NaN if indexing fails.
        """
        try:
...
def test_upper_confidence_bound_with_constraints(gp, constrained_target_space):
    """
    Tests that UpperConfidenceBound acquisition raises ConstraintNotSupportedError when used with a constrained target space.
    """
    acq = acquisition.UpperConfidenceBound()
...
def test_probability_of_improvement(gp, target_space, random_state):
    """
    Tests the ProbabilityOfImprovement acquisition function's parameter handling, error conditions, and exploration decay.
    
    Verifies correct initialization of `xi`, raises an error if `y_max` is unset, checks that suggestions update `xi` according to decay settings, and confirms that no decay leaves `xi` unchanged.
    """
...
def test_probability_of_improvement_with_constraints(gp, constrained_target_space, random_state):
    """
    Tests the ProbabilityOfImprovement acquisition function with constrained target spaces.
    
    Verifies correct error handling when no valid points are registered, and ensures that suggestions are made only when valid points exist under constraints.
    """
    acq = acquisition.ProbabilityOfImprovement(exploration_decay=0.5, exploration_decay_delay=2, xi=0.01)
    assert acq.xi == 0.01
    with pytest.raises(ValueError, match="y_max is not set"):
...
def test_expected_improvement(gp, target_space, random_state):
    """
    Tests the behavior of the ExpectedImprovement acquisition function.
    
    Verifies correct initialization of parameters, error handling when `y_max` is unset, suggestion logic, and the effect of exploration decay on the `xi` parameter.
    """
...
def test_expected_improvement_with_constraints(gp, constrained_target_space, random_state):
    """
    Tests the ExpectedImprovement acquisition function with constrained target spaces.
    
    Verifies correct error handling when no valid points are registered, checks parameter initialization, and ensures that suggestions are made only when valid points exist under constraints.
    """
    acq = acquisition.ExpectedImprovement(exploration_decay=0.5, exploration_decay_delay=2, xi=0.01)
    assert acq.xi == 0.01
    with pytest.raises(ValueError, match="y_max is not set"):
...
def test_constant_liar(gp, target_space, target_func, random_state, strategy):
    """
    Tests the behavior of the Constant Liar acquisition strategy in Bayesian optimization.
    
    Verifies that dummy points are managed correctly, that suggested samples are more diverse than those from the base acquisition, and that new points are registered properly as the optimization progresses.
    """
...
def test_constant_liar_with_constraints(gp, constrained_target_space, random_state):
    """
    Tests ConstantLiar acquisition with constrained target spaces.
    
    Verifies that ConstantLiar raises TargetSpaceEmptyError when no valid points are registered and ConstraintNotSupportedError when constraints are present. Also checks that the base acquisition function produces consistent results.
    """
    base_acq = acquisition.UpperConfidenceBound()
...
def test_gp_hedge(random_state):
    """
    Tests the initialization and base acquisition behavior of the GPHedge acquisition function.
    
    Verifies that GPHedge raises a TypeError when base acquisitions are ambiguous, and checks that its base acquisition functions produce expected outputs when provided with mean and standard deviation arrays.
    """
...
def test_gphedge_update_gains(random_state):
    """
    Tests that GPHedge acquisition correctly updates its internal gains using predictions from a mock Gaussian process.
    """
    base_acq1 = acquisition.UpperConfidenceBound()
    base_acq2 = acquisition.ProbabilityOfImprovement(xi=0.01)
    base_acquisitions = [base_acq1, base_acq2]
    ...
    class MockGP1:
        def __init__(self, n):
            """
            Initializes the object with a gains array of zeros of length n.
            
            Args:
                n: The number of elements in the gains array.
            """
...
def test_gphedge_softmax_sampling(random_state):
    """
    Tests that GPHedge acquisition correctly samples the index of the base acquisition
    with the highest gain using softmax sampling, ensuring deterministic selection
    when one gain is dominant.
    """
    base_acq1 = acquisition.UpperConfidenceBound()
    base_acq2 = acquisition.ProbabilityOfImprovement(xi=0.01)
    base_acquisitions = [base_acq1, base_acq2]
    ...
    class MockGP2:
        def __init__(self, good_index=0):
            """
            Initializes the object with a specified index value.
            
            Args:
                good_index: The index to be stored, defaulting to 0.
            """
...
def test_gphedge_integration(gp, target_space, random_state):
    """
    Tests integration of the GPHedge acquisition function with Bayesian optimization.
    
    Verifies that GPHedge correctly stores its base acquisitions, raises an error when the target space is empty, and can suggest new points after registration. Also checks that new points can be registered iteratively using the suggested parameters.
    """
    base_acq1 = acquisition.UpperConfidenceBound()
    base_acq2 = acquisition.ProbabilityOfImprovement(xi=0.01)
    base_acquisitions = [base_acq1, base_acq2]
...
def test_custom_acquisition_without_get_params():
    ...
    class CustomAcqWithoutGetParams(acquisition.AcquisitionFunction):
        ...
        def base_acq(self, mean, std):
            """
            Computes a simple acquisition value as the sum of mean and standard deviation.
            
            Args:
                mean: The predicted mean value(s).
                std: The predicted standard deviation(s).
            
            Returns:
                The sum of mean and standard deviation, representing a basic acquisition score.
            """
            return mean + std
...
def test_custom_acquisition_without_set_params():
    """
    Tests that a custom acquisition function lacking set_acquisition_params raises NotImplementedError.
    """
